...
export const StockControlView: React.FC<StockControlViewProps> = ({ products, onUpdateProduct, categories, onAddCategory }) => {
    ...
    const renderCell = (col: any, product: any) => {
        ...
        switch (col.key) {
            case 'img':
                return (
                    <div className="h-10 w-10 rounded-lg bg-gray-100 border border-gray-200 flex items-center justify-center overflow-hidden mx-auto">
                        {product.imgUrl ? (
            ...
            case 'stock':
                return (
                    <div className="flex flex-col items-center">
                        <span className={`font-bold text-sm ${available < product.minStock ? 'text-red-600' : 'text-gray-800'}`}>
                            {available} u.
                        </span>
    ...
    const SortableHeader = ({ label, sortKey, currentSort, onSort, className = "", children }: any) => {
        ...
        return (
            <th className={`px-3 py-4 bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors group select-none ${className}`} onClick={() => onSort(sortKey)}
            >
                <div className={`flex items-center gap-1 ${isActive ? 'text-blue-700 font-bold' : 'text-gray-500'} ${className.includes('text-center') ? 'justify-center' : ''}`}>
                    {label}
    ...
    const renderTableHeader = () => {
        if (selectedItems.size > 0) {
            ...
            return (
                <div className="flex items-center justify-between px-6 py-3 bg-blue-50 border-b border-blue-100 animate-fade-in sticky top-0 z-20">
                        ...
                        <div className="flex gap-1 items-center relative">
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className={`p-2 rounded-lg transition-colors flex items-center justify-center ${showBulkActionsMenu ? 'bg-blue-200 text-blue-800' : 'text-blue-700 hover:bg-blue-100'}`}
                                ...
                                {showBulkActionsMenu && (
                                    <div className="absolute left-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 py-2 z-50 animate-zoom-in origin-top-left overflow-hidden">
                                        {/* SECCIÓN 1: ACCIONES MANUALES */}
                                        <div className="px-3 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50/50">
                                            Acciones Manuales
                                        </div>
                                        <button
                                        ...
                                        >
                                            <PauseCircle size={16} className="text-gray-400 group-hover:text-red-500" />
                                            <span>Pausar Selección</span>
                                        </button>
                                        <button
                                            onClick={() => handleBulkStatusChange('active')}
                                            className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-green-50 hover:text-green-600 flex items-center gap-3 transition-colors"
                                        >
                                            <PlayCircle size={16} className="text-gray-400 group-hover:text-green-500" />
                                        ...
                                        >
                                            <RefreshCw size={16} className="text-gray-400 group-hover:text-blue-500" />
                                        ...
                                        <button
                                            ...
                                            className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 flex items-center gap-3 transition-colors"
                                        >
                                            <Wand2 size={16} className="text-gray-400 group-hover:text-blue-500" />
            ...
            <div className="space-y-6 h-full flex flex-col animate-fade-in" onClick={() => {
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between flex-shrink-0 relative z-40 gap-4">
                    <div>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                            <p className="text-sm text-gray-500">Gestión centralizada de productos e inventario.</p>
                            <div className="h-4 w-px bg-gray-300 hidden sm:block"></div>
                            {/* TAB SELECTOR */}
                            <div className="flex items-center gap-1 bg-gray-100/80 p-1.5 rounded-2xl border border-gray-200 shadow-inner ml-1">
                                <button
                                    onClick={() => setActiveTab('inventory')}
                                    className={`px-4 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'inventory' ? 'bg-white text-blue-600 shadow-sm border border-gray-100' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200/50'}`}
                                >
                                    <Package size={14} />
                                    Vista General
                                </button>
                                <button
                                    ...
                                    className={`px-4 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'history' ? 'bg-white text-blue-600 shadow-sm border border-gray-100' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200/50'}`}
                                >
                                    <History size={14} />
                                    Auditoría de Stock
                                </button>
                    ...
                    <div className="relative">
                        <button onClick={(e) => {
                            ...
                            className={`px-4 py-2 rounded-lg flex items-center gap-2 text-sm transition-all shadow-sm border ${showExportMenu ? 'bg-blue-700 text-white border-blue-700 ring-2 ring-blue-100' : 'bg-blue-600 hover:bg-blue-700 text-white border-transparent'}`}
                        >
                            <Download size={16} />
                            ...
                            <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50 animate-zoom-in origin-top-right"
                                ...
                                <div className="px-4 py-3 bg-gray-100 border-b border-gray-200">
                                    <span className="text-xs font-bold text-gray-700 uppercase tracking-wider flex items-center gap-2">
                                        <FileUp size={14} className="text-blue-600" /> Importar (Entrada de datos)
                                    </span>
                                </div>

                                <button className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-blue-600 flex items-center gap-3 transition-colors group">
                                ...
                                <div className="px-4 py-3 bg-gray-100 border-y border-gray-200 mt-2">
                                    <span className="text-xs font-bold text-gray-700 uppercase tracking-wider flex items-center gap-2">
                                        <Download size={14} className="text-green-600" /> Exportar (Salida de datos)
                                    </span>
                                </div>

                                <button className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-green-600 flex items-center gap-3 transition-colors group">
                                    <FileSpreadsheet size={16} className="text-gray-400 group-hover:text-green-600" />
                                ...
                                <button className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-red-500 flex items-center gap-3 transition-colors group">
                ...
                {activeTab === 'inventory' ? (
                    <>
                        ...
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row gap-4 justify-between items-center flex-shrink-0 z-30">
                            ...
                            <div className="flex gap-2 w-full md:w-auto relative">
                                <button onClick={(e) => {
                                    ...
                                    className={`px-4 py-2 border rounded-lg flex items-center gap-2 text-sm flex-1 md:flex-none justify-center transition-colors ${activeFilterCount > 0 || showFilterMenu ? 'border-blue-500 bg-blue-50 text-blue-700 font-medium' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'}`}
                                >
                                    <Filter size={16} />
                                    Filtros
                                ...
                                {showFilterMenu && (
                                    <div className="absolute top-full right-0 mt-2 w-72 md:w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50 p-4 animate-zoom-in"
                                        ...
                                        <div className="flex justify-between items-center mb-4">
                                            ...
                                            {activeFilterCount > 0 && <button onClick={clearFilters} className="text-xs text-red-500 hover:text-red-700 font-medium">Borrar todo</button>}
                                        </div>
                                        ...
                                        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
                                            ...
                                            {/* Filtro: Enlazado */}
                                            <div>
                                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Integraciones</label>
                                                <select
                                                    className="w-full text-sm border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 bg-gray-50"
                                            ...
                                            {/* Filtro: Depósito */}
                                            <div>
                                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Depósito</label>
                                                <select
                                                    className="w-full text-sm border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 bg-gray-50"
                                ...
                                <div className="relative">
                                    <button onClick={(e) => {
                                        ...
                                        className={`p-2 pl-3 pr-2 border rounded-lg flex items-center justify-center gap-1 transition-colors ${showColumnMenu ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 bg-white text-gray-600 hover:bg-gray-50 hover:border-gray-300'}`}
                                            ...
                                            <div className="space-y-1 max-h-[60vh] overflow-y-auto">
                                                {columns.map((col, idx) => (
                                                    <div key={col.key} draggable onDragStart={(e) => handleDragStart(e, idx)}
                                                        ...
                                                        className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors ${col.visible ? 'hover:bg-blue-50' : 'hover:bg-gray-50 text-gray-400'} ${dragItemIndex === idx ? 'opacity-50 border border-dashed border-blue-300 bg-blue-50' : ''}`}
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <span className={`text-sm ${col.visible ? 'text-gray-700 font-medium' : 'text-gray-500'}`}>
                                                                {col.label}
                                                            </span>
                                                        </div>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 flex-1 overflow-hidden flex flex-col relative">
                            ...
                            <div className="overflow-auto flex-1">
                                <table className="w-full text-left border-collapse">
                                    <thead className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wider sticky top-0 z-10 shadow-sm">
                                        <tr>
                                            {/* Checkbox Header */}
                                            <th className={`px-4 py-4 w-10 text-center ${selectedItems.size > 0 ? 'bg-blue-50 border-b border-blue-100' : 'bg-gray-50'}`}>
                                                <input type="checkbox"
                                            ...
                                            {columns.map((col) => {
                                                ...
                                                if (col.sortable) {
                                                    ...
                                                    return (
                                                        <th key={col.key} className={`px-3 py-4 bg-gray-50 ${col.className || ''} ${col.align === 'center' ? 'text-center' : ''} ${col.width || ''}`}>
                                                            {col.label}
                                                        </th>
                                                    );
                                                }
                                    ...
                                    <tbody className="divide-y divide-gray-100 text-sm">
                                        {filtered.map(product => {
                                            ...
                                            return (
                                                <tr key={product.id} className={`transition-colors group ${isSelected ? 'bg-blue-50/60' : 'hover:bg-gray-50'}`}>
                                                    {/* Checkbox Row */}
                            ...
                            <div className="flex-shrink-0 border-t border-gray-100 p-4 bg-gray-50 flex justify-between items-center text-sm text-gray-500">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button className="px-3 py-1 border rounded bg-white hover:bg-gray-50">Siguiente</button>
                                </div>
                            </div>
                        </div>
                    </>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-zoom-in">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                <Bot className="text-blue-600" size={20} /> Automatización Masiva
                            </h3>
                            <button onClick={() => setShowBulkAutomationModal(false)} className="text-gray-400 hover:text-gray-600">
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="p-0 space-y-0">
                            ...
                            <div className="p-6">
                                ...
                                {activeBulkTab === 'pause' ? (
                                    <div className="bg-blue-50/50 rounded-xl p-5 border border-blue-100 animate-slide-in">
                                        <div className="flex items-center justify-between mb-5">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <span className="text-sm font-bold text-gray-800">Pausa Automática</span>
                                                    <p className="text-[10px] text-gray-500 font-medium tracking-tight">Activa la regla en los productos</p>
                                                </div>
                                            </div>
                                            <button
                                            ...
                                            >
                                                {bulkAutoPauseEnabled ? <ToggleRight size={32} className="text-blue-600" /> : <ToggleLeft size={32} className="text-gray-300" />}
                                            </button>
                                        </div>

                                        <div className="space-y-2">
                                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Pausar si stock físico menor a</label>
                                            <div className="relative">
                                                <input
                                                    type="text"
                                                    inputMode="numeric"
                                                    ...
                                                    className={`w-full pl-4 pr-12 py-3 text-xl font-bold border rounded-xl shadow-inner transition-all outline-none ${bulkAutoPauseEnabled
                                                        ? 'bg-white border-blue-200 text-gray-800 focus:border-blue-400'
                                    ...
                                    <div className="bg-orange-50/50 rounded-xl p-5 border border-orange-100 animate-slide-in">
                                        <div className="flex items-center justify-between mb-5">
                                            <div className="flex items-center gap-3">
                                                <div className="p-2 bg-white rounded-lg shadow-sm">
                                                ...
                                                <div>
                                                    <span className="text-sm font-bold text-gray-800">Tope Visible</span>
                                                    <p className="text-[10px] text-gray-500 font-medium tracking-tight">Enmascaramiento de stock real</p>
                                                </div>
                                            </div>
                                            <button
                                                ...
                                                className="focus:outline-none transition-colors"
                                            >
                                                {bulkMaxVisibleEnabled ? <ToggleRight size={32} className="text-orange-600" /> : <ToggleLeft size={32} className="text-gray-300" />}
                                            </button>
                                        </div>

                                        <div className="space-y-2">
                                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Mostrar como máximo</label>
                                            <div className="relative">
                                                <input
                                                    type="text"
                                                    inputMode="numeric"
                                                    ...
                                                    className={`w-full pl-4 pr-12 py-3 text-xl font-bold border rounded-xl shadow-inner transition-all outline-none ${bulkMaxVisibleEnabled
                                                        ? 'bg-white border-orange-200 text-gray-800 focus:border-orange-400'
                                                        : 'bg-gray-100 border-gray-200 text-gray-300 cursor-not-allowed'
                                                        }`}
                                                />
