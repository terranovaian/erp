        ...
        <div className="p-6 bg-slate-50 dark:bg-slate-950 min-h-screen">
            <div className="max-w-4xl mx-auto">
                <header className="mb-10">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">Historial detallado de entradas y salidas de mercancía.</p>
                </header>

                <div className="relative border-l-2 border-slate-200 dark:border-slate-800 ml-5 pl-10 space-y-12">
                    {DUMMY_MOVEMENTS.map((mv) => (
                        <div key={mv.id} className="relative">
                            {/* Icon Circle */}
                            <div className={`absolute -left-[64px] top-0 w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg border-4 border-slate-50 dark:border-slate-950 transition-transform hover:scale-110 z-10
                                ${mv.type === 'IN' ? 'bg-emerald-500 text-white' : 'bg-orange-500 text-white'}
                            `}>
                                {mv.type === 'IN' ? <ArrowDownLeft size={24} /> : <ArrowUpRight size={24} />}
                            </div>

                            <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-xl hover:shadow-gray-200/50 dark:hover:shadow-black/30 transition-all duration-300 group">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-2 h-8 rounded-full ${mv.type === 'IN' ? 'bg-emerald-500' : 'bg-orange-500'}`} />
                                        <h3 className="text-xl font-black text-slate-800 dark:text-slate-100">{mv.productName}</h3>
                                    </div>
                                    <span className={`text-2xl font-black tracking-tighter tabular-nums ${mv.type === 'IN' ? 'text-emerald-600 dark:text-emerald-400' : 'text-orange-600 dark:text-orange-400'}`}>
                                        {mv.type === 'IN' ? '+' : '-'}{mv.quantity} <span className="text-sm font-bold uppercase opacity-60">un.</span>
                                    </span>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4 border-t border-slate-50 dark:border-slate-800">
                                    ...
                                    <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400">
                                        ...
                                        <span className="text-sm font-bold text-slate-700 dark:text-slate-300">{mv.user}</span>
                                    </div>
                                    <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400 md:col-span-1">
                                        <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500">
