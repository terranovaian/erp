...
const SyncChannelCard = ({ channel, generalPrice }) => {
    ...
    const getStatusConfig = (status) => {
        switch (status) {
            case 'active': return {
                label: 'Activa', color: 'bg-green-100 text-green-700 border-green-200', border:
                    'border-l-green-500'
            };
            case 'paused': return {
                label: 'Pausada', color: 'bg-yellow-100 text-yellow-700 border-yellow-200', border:
                    'border-l-yellow-500'
            };
            case 'closed': return {
                label: 'Finalizada', color: 'bg-red-100 text-red-700 border-red-200', border:
                    'border-l-red-500'
            };
            case 'error': return {
                label: 'Error Sync', color: 'bg-orange-100 text-orange-700 border-orange-200',
